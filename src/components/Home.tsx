...
export default function Home({ user, recentOrders, onNavigate, onRefreshData, showToast, onTriggerInstall, isStandalone = false }: HomeProps) {
  ...
  // Parser to extract dynamic remaining data for MTN, GLO, AIRTEL
  const calculateEstimatedDataBalance = (network: 'MTN' | 'GLO' | 'AIRTEL') => {
    // Look at recentOrders (transactions database)
    const successfulNetworkOrders = (recentOrders || []).filter(
      o => o.network === network && o.status === 'success'
    );

    let totalActiveAllocated = 0;
    let totalEstimatedRemaining = 0;
    let latestExpiryDate: Date | null = null;

    const now = new Date().getTime();

    successfulNetworkOrders.forEach(order => {
      const planName = order.plan_name || '';
      // Parse size in GB
      let sizeGB = 0;
      const normalizedName = planName.toLowerCase();
      
      const gbMatch = normalizedName.match(/([\d.]+)\s*gb/);
      if (gbMatch) {
        sizeGB = parseFloat(gbMatch[1]);
      } else {
        const mbMatch = normalizedName.match(/([\d.]+)\s*mb/);
        if (mbMatch) {
          sizeGB = parseFloat(mbMatch[1]) / 1024;
        } else if (normalizedName.includes('1gb')) {
          sizeGB = 1.0;
        } else if (normalizedName.includes('2gb')) {
          sizeGB = 2.0;
        } else if (normalizedName.includes('500mb')) {
          sizeGB = 0.5;
        } else if (normalizedName.includes('230mb')) {
          sizeGB = 0.225;
        } else if (normalizedName.includes('5gb')) {
          sizeGB = 5.0;
        } else if (normalizedName.includes('10gb')) {
          sizeGB = 10.0;
        } else {
          sizeGB = 1.0; // default backup size
        }
      }

      const orderTime = new Date(order.created_at).getTime();
      const thirtyDaysInMs = 30 * 24 * 60 * 60 * 1000;
      const expiryTime = orderTime + thirtyDaysInMs;

      if (expiryTime > now) {
        // Order is active
        totalActiveAllocated += sizeGB;
        // Estimate remaining linearly (linear consumption rate over 30 days)
        const elapsedPercent = (now - orderTime) / thirtyDaysInMs;
        const remainingGB = Math.max(0, sizeGB * (1 - elapsedPercent));
        totalEstimatedRemaining += remainingGB;

        const dateObj = new Date(expiryTime);
        if (!latestExpiryDate || dateObj > latestExpiryDate) {
          latestExpiryDate = dateObj;
        }
      }
    });

    return {
      total: totalActiveAllocated,
      remaining: totalEstimatedRemaining,
      expiry: latestExpiryDate
    };
  };
  ...
  return (
    <PullToRefresh onRefresh={handlePullToRefresh} className="bg-bg-light">
      ...
      {/* Dynamic Active Data Balances Tracker */}
      <div className="px-5 mt-6 shrink-0" id="estimated-data-balances-panel">
        <h4 className="text-xs font-bold text-text-dark/40 uppercase tracking-widest mb-3 pl-1">📡 Active Data Balances</h4>
        
        <div className="bg-white rounded-3xl p-5 border border-gray-100 shadow-sm space-y-4">
          {(['MTN', 'GLO', 'AIRTEL'] as const).map((network) => {
            const dataState = calculateEstimatedDataBalance(network);
            const displayRemaining = dataState.remaining.toFixed(2);
            const displayTotal = dataState.total.toFixed(2);
            const percentage = dataState.total > 0 ? Math.min(100, (dataState.remaining / dataState.total) * 100) : 0;
            
            const networkMeta = {
              MTN: { logoBg: 'bg-yellow-50', iconColor: 'text-yellow-600', textColor: 'text-yellow-800', barColor: 'bg-yellow-400' },
              GLO: { logoBg: 'bg-green-50', iconColor: 'text-green-600', textColor: 'text-green-800', barColor: 'bg-green-500' },
              AIRTEL: { logoBg: 'bg-red-50', iconColor: 'text-red-500', textColor: 'text-red-800', barColor: 'bg-red-500' },
            }[network];
              ...
              <div key={network} className="space-y-2">
                <div className="flex justify-between items-center text-xs">
                  <div className="flex items-center gap-2">
                    <div className={`w-6 h-6 rounded-full ${networkMeta.logoBg} flex items-center justify-center shrink-0`}>
                      <Signal className={`w-3.5 h-3.5 ${networkMeta.iconColor} font-bold`} />
                    </div>
                    <span className="font-extrabold text-primary-dark tracking-wide">{network} Connection</span>
                  </div>
                  <div className="text-right">
                    {dataState.total > 0 ? (
                      <span className="font-bold text-primary-dark font-mono bg-slate-50 border border-slate-100 px-2 py-0.5 rounded-md text-[10.5px]">
                        {displayRemaining} GB / {displayTotal} GB
                      </span>
                    ) : (
                      <span className="text-[10px] uppercase font-bold text-text-muted bg-gray-50 border border-transparent px-2 py-0.5 rounded-md">
                        No Active Pack
                      </span>
                    )}
                  </div>
                ...
                {dataState.total > 0 ? (
                  <div className="space-y-1.5">
                    ...
                    <div className="w-full h-2 bg-gray-100 rounded-full overflow-hidden">
                      <div 
                        className={`h-full transition-all duration-500 rounded-full ${networkMeta.barColor}`}
                        style={{ width: `${percentage}%` }}
                      ></div>
                    </div>
                    <div className="flex justify-between items-center text-[10px] text-text-muted">
                      <span className="font-medium">
                        Estimated Usage: {Math.max(0, 100 - percentage).toFixed(0)}% consumed
                      </span>
                      {dataState.expiry && (
                        <span className="font-bold text-primary-blue">
                          Expires: {dataState.expiry.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
                        </span>
                      )}
                    </div>
                  </div>
                ) : (
                  <div className="flex justify-between items-center bg-gray-50/55 rounded-xl p-2.5 border border-gray-100">
                    <span className="text-[10px] font-medium text-text-muted max-w-[70%] text-left">
                      Activate super cheap MTN, GLO or Airtel data bundles.
                    </span>
                    <button
                      onClick={() => onNavigate('buy_data', { network })}
                      className={`text-[9px] uppercase font-extrabold tracking-wider px-2.5 py-1 rounded-lg border-none text-white bg-primary-dark hover:bg-black transition cursor-pointer shrink-0`}
                    >
                      Buy Data
                    </button>
                  </div>
